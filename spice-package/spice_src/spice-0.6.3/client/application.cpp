...
void Application::prepare_monitors()
{
    ...
    for (int i = 0; i < (int)_screens.size(); i++) {
        ...
        if (_screens[i] && (mon = _screens[i]->get_monitor())) {
            SpicePoint screen_size = _screens[i]->get_size();
            SpicePoint mon_size = mon->get_size();

            if ((screen_size.x > mon_size.x) || (screen_size.y > mon_size.y)) {
                LOG_ERROR("Unable to go fullscreen: Mismatch screen size.");
                LOG_INFO("Monitor Size is: %dx%d", mon_size.x, mon_size.y);
                LOG_INFO("Guest Size is: %dx%d", screen_size.x, screen_size.y);
                throw "Unable to go fullscreen due to: Screen size mismatch.";
            }
...
void Application::enter_full_screen()
{
    ...
    assign_monitors();
    hide();
    // If screen size mismatch happens then return without change.
    try {
        prepare_monitors();
        position_screens();
        show_full_screen();
