...
/*atomiclly add i to the counter and returns the result*/
static inline int _cairo_surface_atomic_add_return(int i, cairo_surface_t *surface)
{
	int __i = i;

	asm volatile("lock; xaddl %0, %1"
		     : "+r" (i), "+m" (surface->ref_count)
		     : : "memory");

	return i + __i;
}

static inline int _cairo_surface_atomic_sub_return(int i, cairo_surface_t *surface)
{
	return _cairo_surface_atomic_add_return(-i, surface);
}

#define cairo_surface_atomic_inc_reference_return(s)  (_cairo_surface_atomic_add_return(1, (s)))
#define cairo_surface_atomic_dec_reference_return(s)  (_cairo_surface_atomic_sub_return(1, (s)))
