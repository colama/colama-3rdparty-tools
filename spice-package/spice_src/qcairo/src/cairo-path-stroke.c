	       ...
	       cairo_stroker_t *stroker, cairo_stroke_face_t *face)
{
    ...
    cairo_fixed_t x_offset, y_offset;
    double half_line_width;
    cairo_fixed_t fixed_width;

    half_line_width = stroker->style->line_width / 2.0;
    fixed_width = _cairo_fixed_from_double (half_line_width);

    if ((fixed_width & 0xffff) == 0x8000)
    {
	x_offset = _cairo_fixed_from_double (abs (slope_dx) * 0.5);
	y_offset = _cairo_fixed_from_double (abs (slope_dy) * 0.5);
    }
    else
    {
	x_offset = 0;
    ...
    {
	face_dx = - slope_dy * half_line_width;
	face_dy = slope_dx * half_line_width;
    }
    else
    {
	face_dx = slope_dy * half_line_width;
    ...
    offset_ccw.x += x_offset;
    offset_cw.x += x_offset;

    offset_ccw.y += y_offset;
    offset_ccw.y += y_offset;
