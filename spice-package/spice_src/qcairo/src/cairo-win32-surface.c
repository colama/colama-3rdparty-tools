...
#ifdef _WIN32_WCE
    // since on most devices the error strings are omitted, don't even try
    // to format error message; just put it out on debug console
    TCHAR szBuffer[512];
    _sntprintf(szBuffer, sizeof(szBuffer)/sizeof(*szBuffer),
       _T("GDI error: 0x%08x, context: %hs\n"),
       last_error,
       context);
				      ...
				      cairo_bool_t needs_scale)
{
	...
	if (!needs_scale && (dst->flags & CAIRO_WIN32_SURFACE_CAN_BITBLT)) {
	    ...
	    BOOL success;
	    int oldmode = SetStretchBltMode(dst->dc, 
#ifdef _WIN32_WCE
                                        0
#else
                                        HALFTONE
...
=======
#ifdef _WIN32_WCE
	win_result = ExtTextOut(dst->dc,
                             start_x,
                             start_y,
                             0/*ETO_GLYPH_INDEX | ETO_PDY*/,
                             NULL,
                             glyph_buf,
                             num_glyphs,
                             dxy_buf);
