...
extern void SendModifierEvents(int flag);
...
DesktopInitBeforeRealization()
{
  ...
  //init custom modifier panel
  CreateModifierPanel(form);
  ...
  XtVaSetValues(desktop, XtNwidth, si.framebufferWidth - 100,
		XtNheight, si.framebufferHeight - 300, NULL);
...
HandleBasicDesktopEvent(Widget w, XtPointer ptr, XEvent *ev, Boolean *cont)
{
  ...
  case LeaveNotify:
	...
	SendKeyEvent(GetModifierEffectiveKeycode(XKeycodeToKeysym(dpy, i, 0)), False);
	modifierPressed[i] = False;
      }
    }
    break;
...
SendRFBEvent(Widget w, XEvent *ev, String *params, Cardinal *num_params)
{
  ...
  if (*num_params != 0) {
	...
	SendKeyEvent(GetModifierEffectiveKeycode(ks), 1);
      } else if (strcasecmp(params[0],"keyup") == 0) {
	SendKeyEvent(GetModifierEffectiveKeycode(ks), 0);
  ...
  case KeyRelease:
    ...
    if (IsModifierKey(ks)) {
      ...
      ks = GetModifierEffectiveKeycode(ks);
