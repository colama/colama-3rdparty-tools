...
class OptionsFrame extends JFrame
  ...
  /**
	 * 
	 */
	private static final long serialVersionUID = -8265740277163342897L;
...
static String[] names = {
  ...
  boolean sixteenBitColors = false;
  boolean thirtytwoBitColors = false;
  ...
  boolean autoScale;
  int scalingFactor;
  boolean debug_optionsFrame = false;
  ...
  OptionsFrame(VncViewer v) {
	  ...
	  choices[encodingIndex].select("Hextile");		// was "Auto"
	  choices[compressLevelIndex].select("Default");
	  choices[jpegQualityIndex].select("6");
	  choices[cursorUpdatesIndex].select("Enable");
	  choices[useCopyRectIndex].select("Yes");
	  choices[eightBitColorsIndex].select("16bit");	// was "No"
	  choices[mouseButtonIndex].select("Normal");
	  choices[viewOnlyIndex].select("No");
	  choices[scaleCursorIndex].select("No");
	  choices[shareDesktopIndex].select("No");		// was "Yes"
	  choices[debug_optionsFrameIndex].select("Yes");
  ...
  void setEncodings() {
	  if (debug_optionsFrame) System.out.println("in OptionsFrame.setEncodings() ...");
	  
	  useCopyRect = choices[useCopyRectIndex].getSelectedItem().equals("Yes");
	  if (debug_optionsFrame) System.out.println("  useCopyRect = " + useCopyRect);
	  ...
	  if (debug_optionsFrame) System.out.println("  preferredEncoding = " + preferredEncoding);
	  ...
	  if (debug_optionsFrame) System.out.println("  compressLevel = " + compressLevel);
	  ...
	  if (debug_optionsFrame) System.out.println("  jpegQuality = " + jpegQuality);
	  ...
	  if (debug_optionsFrame) System.out.println("  requestCursorUpdates = " + requestCursorUpdates);
	  if (debug_optionsFrame) System.out.println("  ignoreCursorUpdates = " + ignoreCursorUpdates);
  ...
  void setColorFormat() {
	  ...
	  sixteenBitColors =
		  choices[eightBitColorsIndex].getSelectedItem().equals("16bit");
	  thirtytwoBitColors =
		  choices[eightBitColorsIndex].getSelectedItem().equals("32bit");
  ...
  void setOtherOptions() {
	  ...
	  debug_optionsFrame
	  = choices[debug_optionsFrameIndex].getSelectedItem().equals("Yes");
  ...
  public void itemStateChanged(ItemEvent evt) {
	  ...
	  if (source == choices[encodingIndex] ||
		  ...
		  setColorFormat();
			  ...
			  source == choices[debug_optionsFrameIndex]) {
