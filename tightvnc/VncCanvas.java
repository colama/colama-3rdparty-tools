...
class VncCanvas extends JPanel
implements KeyListener, MouseListener, MouseMotionListener, MouseWheelListener {
	...
	VncViewer viewer;
	RfbProto rfb;
	ColorModel cm8, cm24, cm16;
	Color[] colors;
	int bytesPixel;
	boolean debug_vncCanvas = false;
	...
	byte[] pixels8;
	int[] pixels24;
	int[] pixels16;  // should have been ushort, but MemoryImageSource can't handle ushorts
	...
	boolean debug_keyhandler = false;
	boolean debug_mousehandler = false;
	...
	throws IOException {
		...
		cm16 = new DirectColorModel(24, 0x0000F800, 0x000007E0, 0x0000001F);
...
//	}
	...
	public boolean imageUpdate(Image img, int infoflags,
		...
		if ((infoflags & (ALLBITS | ABORT)) == 0) {
			...
			if ((infoflags & ALLBITS) != 0) {
				if (jpegRect != null) {
					if (debug_vncCanvas) System.out.println("img update");
	...
	public void setPixelFormat() throws IOException {
		if (viewer.options.eightBitColors) {
			...
			bytesPixel = 1;
		} else if (viewer.options.sixteenBitColors) {
			// setting the Pixel format upsets the vncserver, so avoiding it
			// rfb.writeSetPixelFormat(16, 16, false, true, 31, 63, 31, 11, 5, 0);
			bytesPixel = 2;	  
		} else { // (viewer.options.thirtytwoBitColors)
			rfb.writeSetPixelFormat(32, 24, false, true, 255, 255, 255, 16, 8, 0);
			bytesPixel = 4;  
		}
		if (debug_vncCanvas) System.out.println("setting pixelformat. bytesPixel = " + bytesPixel);
		updateFramebufferSize();
	}

	public void setPixelFormat(int bpp) throws IOException {
		if(bpp/8 != bytesPixel) {
			bytesPixel = bpp/8;
			if (debug_vncCanvas) System.out.println("setting pixelformat. bytesPixel = " + bytesPixel);
			updateFramebufferSize();
	...
	void updateFramebufferSize() {
		...
		if (maxWidth > 0 && maxHeight > 0) {
			...
			if (scalingFactor > 100)
				scalingFactor = 100;
			if (debug_vncCanvas) System.out.println("Scaling desktop at " + scalingFactor + "%");
		}
		...
		if (bytesPixel == 1) {
			...
			pixels8 = null;
			pixels16 = new int[fbWidth * fbHeight]; // should have been ushort but MemoryImageSource can't handle it
			pixels24 = null;

			pixelsSource =
				new MemoryImageSource(fbWidth, fbHeight, cm16, pixels16, 0, fbWidth);

			zrleTilePixels8 = null;
			zrleTilePixels16 = new short[64 * 64];
			zrleTilePixels24 = null;
	...
	public void processNormalProtocol() throws Exception {
		if (debug_vncCanvas) System.out.println("processing processNormalProtocol");
		// Start/stop session recording if necessary.
		viewer.checkRecordingStatus();
		setPixelFormat(rfb.bitsPerPixel);

		rfb.writeFramebufferUpdateRequest(0, 0, rfb.framebufferWidth,
				rfb.framebufferHeight, false);

		rfb.sendFakeModifiersRelease();
		...
		while (true) {
			...
			case RfbProto.FramebufferUpdate:
				...
				for (int i = 0; i < rfb.updateNRects; i++) {
					...
					if (rfb.updateRectEncoding == RfbProto.EncodingXCursor ||
						...
						if (debug_vncCanvas) System.out.println("Got Xcursor update");
						handleCursorShapeUpdate(rfb.updateRectEncoding, rx, ry, rw, rh);
						continue;
					}

					if (rfb.updateRectEncoding == RfbProto.EncodingPointerPos) {
						if (debug_vncCanvas) System.out.println("Got cursorPos update");
					...
					case RfbProto.EncodingRaw:
						if (debug_vncCanvas) System.out.println("Encoding: Raw");
						handleRawRect(rx, ry, rw, rh);
						break;
					case RfbProto.EncodingCopyRect:
						if (debug_vncCanvas) System.out.println("Encoding: CopyRect");
					...
					case RfbProto.EncodingHextile:
						if (debug_vncCanvas) System.out.println("Encoding: Hex");
					...
					case RfbProto.EncodingTight:
						 ...
						 */
					default:
						throw new UnknownRFBEntityException("Rectangle Encoding", 
			...
			case RfbProto.ServerCutText:
				if (debug_vncCanvas) System.out.println("ServerCuttext");
				String s = rfb.readServerCutText();
				viewer.clipboard.setCutText(s);
				break;

			default:
				throw new UnknownRFBEntityException("Message Type", msgType);
			}
		}
	}
	...
	throws IOException {
		if (debug_vncCanvas) System.out.println("Processing handleRawRect\n\t bytesPixel = " + bytesPixel + ", paint = " +  paint);
		if (debug_vncCanvas) System.out.println("\tx = " + x + ", y = " + y + ", w = " + w + ", h = " + h);
		if (bytesPixel == 1) {
			...
			for (int dy = y; dy < y + h; dy++) {
				...
				for (i = 0; i < w; i++) {
					int tmp = 0;
					tmp |= ((buf[i*2+1] & 0xFF) << 8) | (buf[i*2] & 0xFF);
					pixels16[offset + i] = tmp; 
				}
			}
		} else { // bytesPixel == 4
			assert bytesPixel == 4 : "unexpected bytesPixel value" ;
	...
	void handleHextileRect(int x, int y, int w, int h) throws IOException {
		if (debug_vncCanvas) System.out.println("Processing handleHextileRect\n\tbytesPixel = " + bytesPixel);
		if (debug_vncCanvas) System.out.println("\tx = " + x + ", y = " + y + ", w = " + w + ", h = " + h);
		hextile_bg = new Color(0);
		hextile_fg = new Color(0);

		for (int ty = y; ty < y + h; ty += 16) {
			int th = 16;			// the default height of a hextile
			if (y + h - ty < 16) {	// fix the height of the last row to fill 
				th = y + h - ty;
			}

			for (int tx = x; tx < x + w; tx += 16) {
				int tw = 16;			// the default width of a hextile
				if (x + w - tx < 16) {	// fix the width of the last row to fill
					tw = x + w - tx;	
				}
	...
	throws IOException {
		if (debug_vncCanvas) System.out.println("Processing handleHextileSubrect\n\tbytesPixel = " + bytesPixel);
		int subencoding = rfb.is.readUnsignedByte();
		if (rfb.rec != null) {
			rfb.rec.writeByte(subencoding);
		}
		if (debug_vncCanvas) System.out.println("\tsubencoding = " + subencoding + ", x = " + tx + ", y = " + ty + ", w = " + tw + ", h = " + th);
		...
		if ((subencoding & RfbProto.HextileBackgroundSpecified) != 0) {
			...
			if (bytesPixel == 1) {
				...
				int tmp = 0;
				tmp |= ((cbuf[1] & 0xFF) << 8) | (cbuf[0] & 0xFF);
				hextile_bg = new Color(cm16.getRGB(tmp));
			} else { // bytesPixel == 4
				assert bytesPixel == 4 : "unexpected bytesPixel value" ;
		...
		if ((subencoding & RfbProto.HextileForegroundSpecified) != 0) {
			...
			if (bytesPixel == 1) {
				...
				int tmp = 0;
				tmp |= ((cbuf[1] & 0xFF) << 8) | (cbuf[0] & 0xFF);
				hextile_fg = new Color(cm16.getRGB(tmp));
			} else { // bytesPixel == 4
				assert bytesPixel == 4 : "unexpected bytesPixel value" ;
		...
		if ((subencoding & (RfbProto.HextileForegroundSpecified | RfbProto.HextileBackgroundSpecified)) != 0)
			if (debug_vncCanvas) System.out.println("\tfg = " +  hextile_fg + ", bg = " + hextile_bg);

		// Done with this tile if there is no sub-rectangles.
		if ((subencoding & RfbProto.HextileAnySubrects) == 0) {
			if (debug_vncCanvas) System.out.println("\tskipping");
			return;
		}

		int nSubrects = rfb.is.readUnsignedByte();
		int bufsize = nSubrects * 2;
		if (debug_vncCanvas) System.out.println("\t\tprocessing subrects nSubrects = " + nSubrects);

		if ((subencoding & RfbProto.HextileSubrectsColoured) != 0) {
			bufsize += nSubrects * bytesPixel;
			if (debug_vncCanvas) System.out.println("\t\tprocessing coloured subrects");
		}
		...
		if ((subencoding & RfbProto.HextileSubrectsColoured) == 0) {
			...
			if (debug_vncCanvas) System.out.println("\t\t\tfg = " +  hextile_fg);
			...
			// Full-color (16-bit) version for colored sub-rectangles.
			for (int j = 0; j < nSubrects; j++) {
				int tmp = 0;
				tmp = ((buf[i+1] & 0xFF) << 8 | (buf[i] & 0xFF));
				hextile_fg = new Color(cm16.getRGB(tmp));
				i += 2;
				b1 = buf[i++] & 0xFF;
				b2 = buf[i++] & 0xFF;
	...
	void handleZRLERect(int x, int y, int w, int h) throws Exception {
		...
		if (rfb.rec != null) {
			if (rfb.recordFromBeginning) {
				...
				if (debug_vncCanvas) System.out.println("Warning: ZRLE session can be recorded" +
				" only from the beginning");
				if (debug_vncCanvas) System.out.println("Warning: Recorded file may be corrupted");
	...
	throws IOException {
		...
		if (debug_vncCanvas) System.out.println("processing handleCursorShapeUpdate");
		softCursorFree();
	...
	throws IOException {
		if (debug_vncCanvas) System.out.println("processing decodeCursorShape");
		if (debug_vncCanvas) System.out.println("\t bytesPixel = " +  bytesPixel);
		...
		if (encodingType == RfbProto.EncodingXCursor) {
			if (debug_vncCanvas) System.out.println("\t handling EncodingXCursor");
			...
			if (debug_vncCanvas) System.out.println("\t handling EncodingRichCursor");
					...
					for (n = 7; n >= 0; n--) {
						if ((maskByte >> n & 1) != 0) {
							if (bytesPixel == 1) {
								...
								int tmp = 0;
								tmp |= ((pixBuf[i * 2 + 1] & 0xFF) << 8) | (pixBuf[i * 2] & 0xFF);
								result = 0xFF000000 | cm16.getRGB(tmp);
							} else { 
								assert bytesPixel == 4 : "Unexpected value for bytesPixel" ;
								result = 0xFF000000 |
	...
	createSoftCursor() {
		...
		if (softCursorSource == null)
			return;
		if (debug_vncCanvas) System.out.println("processing createSoftCursor");
	...
	synchronized void softCursorMove(int x, int y) {
		...
		cursorY = y;
		if (showSoftCursor) {
			if (debug_vncCanvas) System.out.println("processing softCursorMove");
			repaint(viewer.deferCursorUpdates,
	...
	synchronized void softCursorFree() {
		if (debug_vncCanvas) System.out.println("processing softCursorFree");
		if (showSoftCursor) {
	...
	public void showSplKeysMenu () {
		// 
		// puts up the menu panel, resets the checkbox button states, and 
		// resets the state of modifiers
		//
		viewer.vncFrame.add(viewer.modifierMenu.mainModifierPanel);
		
		if (viewer.transposeRightMods == true) {
			viewer.modifierMenu.transposeRightMods.setSelected(true);
		} else
			viewer.modifierMenu.transposeRightMods.setSelected(false);
		
		viewer.vncFrame.validate();
		viewer.modifierMenu.resetAll();
	}

	public void hideSplKeysMenu () {
		// 
		// tears down the menu panel, resets the checkbox button states, and 
		// resets the state of modifiers
		//

		try {
			viewer.vncFrame.remove(viewer.modifierMenu.mainModifierPanel);
			viewer.vncFrame.validate();
			viewer.modifierMenu.resetAll();
		} catch (Exception e) {
			if (debug_keyhandler) System.out.println("Err hiding frame: " + e);
		}
	}
