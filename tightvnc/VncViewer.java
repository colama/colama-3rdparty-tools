...
implements java.lang.Runnable, WindowListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3644014979062532320L;
	boolean inAnApplet = true;
	boolean inSeparateFrame = true;

	boolean shudExit = false;
	int count = 0;
	...
	CmdPopMenu modifierMenu;
	String jarRevisionStr;
	boolean transposeRightMods;
	...
	public void init() {
		...
		if (inSeparateFrame) {
			vncFrame = new JFrame("TightVNC");
			if (!inAnApplet) {
				vncFrame.getContentPane().add("Center", this);
			}
		...
		try { // accessing the Manifest
			...
			int mark = pathToThisClass.length() -  classFileName.length() - 1;
			String pathToManifest = pathToThisClass.toString().substring(0, mark) + 
				"/META-INF/MANIFEST.MF" ;
			URL url = new URL(pathToManifest);
			JarURLConnection jarConnection = (JarURLConnection) url.openConnection();
			Manifest manifest = jarConnection.getManifest();

			Attributes attributes = manifest.getAttributes("version-info");
			jarRevisionStr = attributes.getValue("revision-number");
			
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		transposeRightMods = true;
	...
	private void do_work() throws Exception {
		...
		if (inSeparateFrame) {
			...
			desktopScrollPane = new JScrollPane();
			gbc.fill = GridBagConstraints.BOTH;
			gridbag.setConstraints(desktopScrollPane, gbc);
			desktopScrollPane.setBorder(BorderFactory.createEmptyBorder());
			desktopScrollPane.getViewport().add(canvasPanel);

			// Finally, add our ScrollPane to the Frame window.
			vncFrame.getContentPane().add(desktopScrollPane);
		...
		modifierMenu = new CmdPopMenu(this);
		modifierMenu.modifierKeyPanel.setVisible(true);

		if (showControls)
			buttonPanel.enableButtons();

		moveFocusToDesktop();
		vncFrame.setExtendedState(JFrame.NORMAL);
		count = 0;
	...
	public void run() {
		...
		while(!shouldExit) {
			try {
				vncFrame.getContentPane().removeAll();
				resetSettings();
				do_work();
			} catch (UnknownRFBEntityException e) {
				if (shudExit == true)
					break;
				System.out.println("Got Exception: " + e);
				rfb.close();
				continue;
			} catch (FBRectUpdateTooLargeException e) {
				if (shudExit == true)
					break;
				System.out.println("Got Exception: " + e);
				rfb.close();
...
//	reset all setting set during protocol init
	void resetSettings() {
		resetSavedEncodings();
	}
	//
	...
	//resets saved encoding. 
	//Used when re-connected to server
	void resetSavedEncodings() {
		nEncodingsSaved = 0;
		encodingsSaved = null;
	}
