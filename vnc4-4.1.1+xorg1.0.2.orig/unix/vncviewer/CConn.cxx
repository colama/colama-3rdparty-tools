...
StringParameter menuKey("MenuKey", "The key which brings up the popup menu",
                        "");
StringParameter modPanelKey("ModPanelKey", "The key which brings up modifier panel",
                        "Alt_L");
...
CConn::CConn(Display* dpy_, int argc_, char** argv_, network::Socket* sock_,
  ...
  : dpy(dpy_), argc(argc_),
    ...
    modPanel(dpy, this), modPanelEventHandler(0), reverseConnection(reverse)
{
  CharArray menuKeyStr(menuKey.getData());
  menuKeysym = XStringToKeysym(menuKeyStr.buf);
  CharArray modPanelKeyStr(modPanelKey.getData());
  modPanelKeysym = XStringToKeysym(modPanelKeyStr.buf);
  modPanel.addEventMask(KeyPressMask | KeyReleaseMask);
  modPanelEventHandler = modPanel.setEventHandler(this);
  memset(&modPanelTime, 0, sizeof(modPanelTime));
...
void CConn::handleModPanelEvent(TXWindow* w, XEvent* ev)
{
    KeySym ks=0;
    char str[256];
    if(ev->type != KeyPress)
    ...
    if(ks == XK_Alt_L)
        modPanel.al.checked(!modPanel.al.checked());
    else if(ks == XK_Alt_R)
        modPanel.ar.checked(!modPanel.ar.checked());
    else if(ks == XK_Control_L)
        modPanel.cl.checked(!modPanel.cl.checked());
    else if(ks == XK_Control_R)
        modPanel.cr.checked(!modPanel.cr.checked());
    else if(ks == XK_Shift_L)
        modPanel.sl.checked(!modPanel.sl.checked());
    else if(ks == XK_Shift_R)
        modPanel.sr.checked(!modPanel.sr.checked());
    else if(ks == XK_Super_L)
        modPanel.wk.checked(!modPanel.wk.checked());
}
                                           

// get effective modifier key
KeySym CConn::getEffectiveModifierKey(KeySym ks)
{
    if(modPanel.trp.checked())
    {
        if(ks == XK_Alt_R)
            return XK_Alt_L;
        if(ks == XK_Control_R)
...
//send appropriate modifier events
void CConn::sendModifierEvents(bool flag)
{
    if(flag)
    {
        if(modPanel.al.checked())
            writer()->keyEvent(XK_Alt_L, true);
        if(modPanel.ar.checked())
            writer()->keyEvent(getEffectiveModifierKey(XK_Alt_R), true);
        if(modPanel.cl.checked())
            writer()->keyEvent(XK_Control_L, true);
        if(modPanel.cr.checked())
            writer()->keyEvent(getEffectiveModifierKey(XK_Control_R), true);
        if(modPanel.sl.checked())
            writer()->keyEvent(XK_Shift_L, true);
        if(modPanel.sr.checked())
            writer()->keyEvent(getEffectiveModifierKey(XK_Shift_R), true);
        if(modPanel.wk.checked())
            writer()->keyEvent(XK_Super_L, true);
    }
    else 
    {
        if(modPanel.wk.checked())
            writer()->keyEvent(XK_Super_L, false);
        if(modPanel.sr.checked())
            writer()->keyEvent(getEffectiveModifierKey(XK_Shift_R), false);
        if(modPanel.sl.checked())
            writer()->keyEvent(XK_Shift_L, false);
        if(modPanel.cr.checked())
            writer()->keyEvent(getEffectiveModifierKey(XK_Control_R), false);
        if(modPanel.cl.checked())
            writer()->keyEvent(XK_Control_L, false);
        if(modPanel.ar.checked())
            writer()->keyEvent(getEffectiveModifierKey(XK_Alt_R), false);
        if(modPanel.al.checked())
...
void CConn::handleEvent(TXWindow* w, XEvent* ev)
{
  ...
  case KeyRelease:
    ...
    {
        if(IsModifierKey(ks))
            handleModPanelEvent(w, ev);
        else
        {
            desktopEventHandler->handleEvent(w, ev);
            if(fullScreen)
                XGrabKeyboard(dpy, desktop->win(), True, GrabModeAsync, 
    ...
    if (ks == menuKeysym && (ev->xkey.state & (ShiftMask|ControlMask)) == 0) {
      if (w == desktop && ev->type == KeyPress) {
        /* 
         * Commenting this line to remove F8 menu.
    ...
    else if (ks == modPanelKeysym && ev->type == KeyPress) {
        struct timeval t2;
        gettimeofday(&t2, NULL);
        if(t2.tv_sec == modPanelTime.tv_sec && 
            t2.tv_usec - modPanelTime.tv_usec <= 500 * 1000)
        {
            modPanel.showPanel(ev->xkey.x_root, ev->xkey.y_root);
            if(fullScreen)
                XGrabKeyboard(dpy, modPanel.win(), True, GrabModeAsync, 
                            GrabModeAsync, CurrentTime);
        }
	else
	{
...
void CConn::toggleFullScreen() {

    fullScreen = !fullScreen;
    if (viewport) recreateViewport();
    if(fullScreen)
        modPanel.fscr.setText("FullScreen Off");
    else
        modPanel.fscr.setText("FullScreen On");
}
//sent keypress + release for all modifiers
void CConn::resetAllModifiers() {

    writer()->keyEvent(XK_Alt_L, true);
    writer()->keyEvent(XK_Alt_L, false);
    writer()->keyEvent(XK_Alt_R, true);
    writer()->keyEvent(XK_Alt_R, false);
    writer()->keyEvent(XK_Control_L, true);
    writer()->keyEvent(XK_Control_L, false);
    writer()->keyEvent(XK_Control_R, true);
    writer()->keyEvent(XK_Control_R, false);
    writer()->keyEvent(XK_Shift_L, true);
    writer()->keyEvent(XK_Shift_L, false);
    writer()->keyEvent(XK_Shift_R, true);
    writer()->keyEvent(XK_Shift_R, false);
    writer()->keyEvent(XK_Super_L, true);
    writer()->keyEvent(XK_Super_L, false);
    if (fullScreen) {
...
void CConn::recreateViewport()
{
  ...
  XChangeWindowAttributes(dpy, modPanel.win(), CWOverrideRedirect, &attr);
  reconfigureViewport();
